...
export function EditStoreView() {
  ...
  const [nameAr, setNameAr] = useState(selectedStore.nameAr);
  const [description, setDescription] = useState(selectedStore.description);
  const [descriptionAr, setDescriptionAr] = useState(
    selectedStore.descriptionAr
  );
  const [phoneNumber, setPhoneNumber] = useState(selectedStore.phoneNumber);
  const [order, setOrder] = useState(selectedStore.order);
  const [likes, setLikes] = useState(selectedStore.likes);
  const [costs, setCosts] = useState(selectedStore.costs);
  const [open, setOpen] = useState(selectedStore.open);
  const [selectedTags, setSelectedTags] = useState(selectedStore.tags);
  const [latitude, setLatitude] = useState(selectedStore.latitude);
  const [longitude, setLongitude] = useState(selectedStore.longitude);
  ...
  const handleSubmitStore = () => {
    const body = {
      ...
      latitude,
      longitude,
      image: createStoreImageId || selectedStore.image,
    ...
    selectedStore && (
      <div style={styles.mainContainer}>
        ...
        <div style={styles.fileContainer}>
          ...
          />
          {(createStoreImageId || selectedStore.image) && (
            <img
              alt=""
              style={styles.image}
              src={`${appConfig.apiUrl}/api/static/stores/${
                createStoreImageId || selectedStore.image
              }`}
            />
          )}
